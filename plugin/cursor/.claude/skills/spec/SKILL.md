...
1. Spawns **`pmo`**, which **converses** with you — stating the intent (the user-observable change) before any implementation talk, then debating decisions and edge cases, recording the *why*. Where the questions are structural (what exists, what depends on what), it grounds them per `.claude/rules/code-query.md` — graph first, grep second — instead of guessing.
