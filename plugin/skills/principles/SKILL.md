...
2. **Already in this codebase?** Reuse it — the `code-query` skill finds it.
...
on this, how are these connected), the `code-query` skill finds the callers
cheaply — graph first, grep second; then read what it surfaces.
