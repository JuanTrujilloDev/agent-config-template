...
- Every implementer prompt carries the `TDD quality guardrails` from the `sdd-workflow` skill plus the `agent_style` line above; do not duplicate the full policy in each stack agent.
