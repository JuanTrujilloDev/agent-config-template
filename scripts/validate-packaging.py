#!/usr/bin/env python3
"""Validate that the distributable agent plugins are install-ready.
...
cursor_manifest = load(".cursor-plugin/plugin.json")
if cursor_manifest:
    for k in ("name", "version", "description", "rules", "agents", "skills", "commands", "hooks"):
        if k not in cursor_manifest:
            err(f"Cursor plugin.json missing '{k}'")
    versions["Cursor plugin.json"] = cursor_manifest.get("version")
    for field in ("rules", "agents", "skills", "commands", "hooks"):
        value = cursor_manifest.get(field)
        paths = value if isinstance(value, list) else [value]
        for rel in paths:
            if not isinstance(rel, str) or os.path.isabs(rel) or rel == ".." or rel.startswith("../"):
                err(f"Cursor plugin.json field '{field}' must use safe relative paths")
            elif not os.path.exists(os.path.join(REPO, rel)):
                err(f"Cursor plugin.json field '{field}' does not resolve: {rel}")
...
def walk_components(base, kind, require=None, allow_requires=False, need_disable=False):
    ...
    if kind == "skills":
        ...
        for fn in sorted(os.listdir(d)):
            if fn.endswith(".md"):
                req = require or (("name", "description") if kind == "agents" else ("description",))
...
for rel in ("plugin/commands/setup-template.md", "plugin/commands/setup-companions.md"):
    parse_frontmatter(os.path.join(REPO, rel), rel, require=("name", "description"))
walk_components("core/.claude/agents", "agents")
walk_components("core/.claude/commands", "commands")

cursor_rule_dir = os.path.join(REPO, "plugin/.cursor-plugin/rules")
if os.path.isdir(cursor_rule_dir):
    for fn in sorted(os.listdir(cursor_rule_dir)):
        if fn.endswith(".mdc"):
            parse_frontmatter(os.path.join(cursor_rule_dir, fn),
                              f"plugin/.cursor-plugin/rules/{fn}")
cursor_hooks = load("plugin/.cursor-plugin/hooks.json")
if cursor_hooks:
    for definitions in (cursor_hooks.get("hooks") or {}).values():
        for definition in definitions:
            command = definition.get("command", "")
            if not command.startswith("./") or not os.path.isfile(os.path.join(REPO, command[2:])):
                err(f"Cursor plugin hook command does not resolve: {command}")
for rel in ("plugin/.cursor-plugin/hooks/branch-guard.sh",
            "plugin/.cursor-plugin/hooks/format-on-edit.sh"):
    path = os.path.join(REPO, rel)
    if not os.path.isfile(path) or not os.access(path, os.X_OK):
        err(f"Cursor plugin hook missing or not executable: {rel}")
...
print(f"packaging valid @ v{v}: Claude + Cursor + Codex plugin manifests, "
      f"agents, commands, skills, rules, hooks, and rendered host trees ✓")
