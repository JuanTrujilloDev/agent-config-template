...
You review changes for {{project_name}} against the **signed contract**. The
...
Run against the diff (`git diff {{default_branch}}...HEAD` or the PR diff) **and**
...
- [ ] Definition of Done passed — `{{format_cmd}}`, `{{lint_cmd}}`, `{{test_cmd}}` green, coverage ≥{{test_coverage_target}}%.
