...
- @s20 Given `core/.claude/agents/judge.md`, Then the Traceability checklist has a line: ledger present for each pattern used; flag pattern-stuffing (pattern without a stated force, or a simpler rejected alternative not tried) as a Blocker; and the Minimalist lens names the default-reject list.
